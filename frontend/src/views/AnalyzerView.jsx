                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="font-display font-bold text-slate-900 text-sm flex items-center gap-2">
                      ...
                      Top Recommended Titles ({result.recommendations.length} Matches)
                    </h4>
                    <span className="text-[11px] text-emerald-600 font-mono font-bold bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">
                      ● Sorted by Match Score & Year
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {result.recommendations.map((rec, idx) => (
                      <motion.div 
                        ...
                        className="p-4.5 rounded-3xl floating-card space-y-3 relative overflow-hidden bg-white border border-slate-200/90 shadow-sm hover:border-indigo-300 transition-all flex flex-col justify-between"
                      >
                        <div className="space-y-2">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <span className="px-2 py-0.5 rounded-md bg-indigo-100 text-indigo-800 text-[10px] font-mono font-bold border border-indigo-200">
                                {rec.badge || "FEATURED"}
                              </span>
                              <h5 className="font-display font-extrabold text-base text-slate-900 mt-1 leading-snug">{rec.title}</h5>
                            </div>

                            <span className="px-2.5 py-1 rounded-xl bg-slate-900 text-amber-300 text-xs font-mono font-bold shrink-0 shadow-sm flex items-center gap-1">
                              <span>★</span>
                              <span>{rec.rating || 9.2}</span>
                            </span>
                          </div>

                          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 font-mono">
                            <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 font-bold">{rec.release_year || 2026}</span>
                            <span>•</span>
                            ...
                            <span>•</span>
                            <span className="text-indigo-600 font-bold">{rec.content_type}</span>
                          </div>
                        ...
                        <div className="space-y-2 pt-2 border-t border-slate-100">
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-slate-500 font-mono text-[11px]">Match Score</span>
                            <span className="font-mono font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">
                              {rec.match_percentage || 96}% MATCH
                            </span>
                          </div>

                          <div className="p-2.5 rounded-xl bg-slate-50 text-[11px] text-slate-700 border border-slate-200 leading-relaxed">
                            <span className="font-bold text-indigo-600 font-display">Match Logic: </span>
                            {rec.match_reason}
                          </div>
                        </div>
                      </motion.div>
